...
def main(): 
    choose = option_menu("Menu", ["Home", "Identifikasi", "Jenis Motif", "Tentang", "TM Canny"],
            ...
            styles={
                "container": {"padding": "5!important", "background-color": "#f6cc53"}, #pengaturan tampilan
    ...
    if choose == "Home": 
        profile = Image.open("./Images/demo.jpg") 
    ...
    #deteksi otomatis
        ...
        # )
        # Detect edges button
        edge_button = st.button("Deteksi") #tombol deteksi
        ...
        # Edge Image
        if edge_button: #jika tombol deteksi ditekan maka alur program lanjut ke sini
            array = np.asarray(image)
            gray = cv2.cvtColor(array, cv2.COLOR_BGR2GRAY)
            blurry = cv2.GaussianBlur(gray, (5, 5), 0)#gambar grayscale diproses dengan fungsi GaussianBlur
            low_thresh = 0 
            upper_thresh = 0 
            if low_thresh == 0 and upper_thresh == 0:#proses canny
                edged = cv2.Canny(blurry, 15, 200)#penentuan upper tresh dan lower tresh otomatis
            else:
            ...
            try:
                with st.spinner("Mendeteksi...."):
                    label = our_image_classifier(image)#pemanggilan class
                    nilaiprediksi = nilai_prediksi(image)
                    time.sleep(8)
                st.success("Motifnya adalah: "+label)#hasil prediksi berupa jenis motif
                ##st.markdown("Nilai deteksi: "+nilaiprediksi+" x 100")# hasil predikisi berupa angka
    ...
    # halaman tentang
        ...
        col1,col2,col3= st.columns(3)
        with col1:st.image("./Images/foto_about.jpg", width=200 )
        with col2:
    ...
    #Menu Template Matching
        ...
        if tm_button:
            ...
            res = cv2.matchTemplate(edged,edged_template,cv2.TM_CCOEFF_NORMED)#template matching
            (_, max_val, _, max_loc) = cv2.minMaxLoc(res)#template matching
            
            # Specify a threshold
            threshold = 0.2
            loc = np.where( res >= threshold)#jika ada bagian yang terdeteksi akan muncul kotak merah
            for pt in zip(*loc[::-1]):
                cv2.rectangle(array, pt, (pt[0] + w, pt[1] + h), (255,0,0), 2)#menampilkan kotak dibagian yang terdeteksi
